...
menu_music_tracks = ["sound/Main_menu_1.mp3", "sound/Main_menu_1.mp3"]
fight_music_tracks = ["sound/Fight_music_1.mp3"]
current_menu_track = None  # Stores the currently loaded main menu track

# GLOBAL SOUND LISTS
hurt_sounds = ["sound/Hurt_1.mp3", "sound/Hurt_2.mp3"]
death_sounds = ["sound/Death_1.mp3", "sound/Death_1.mp3"]
...
heart_frames = extract_frames("images/heart.gif", 50, 50)
...
class CentaurSpear(Centaur):
    def __init__(self, pos, speed, health=150):
        image = pygame.image.load("images/centaur_spear.png")
...
def starting_room():
    floor = pygame.image.load("images/pixel_floor.jpeg")
    floor = pygame.transform.scale(floor,(width,height))
    SCREEN.blit(floor,(0,0))
# Scaling & creating weapon images
    bow_img = pygame.image.load("Weapons/bow.png")
    sword_img = pygame.image.load("Weapons/sword.png")
    spear_img = pygame.image.load("Weapons/spear.png")
    spell_img = pygame.image.load("Weapons/spellbook.png")
    shield_img = pygame.image.load("Weapons/shield.png")
    spell_img = pygame.transform.scale2x(spell_img)
# Weapon img positions
    bow_pos = (200,200)
    sword_pos = (width - 400,200)
    spear_pos = (200,height - 400)
    spell_pos = (width / 2 - 300,200)
    shield_pos = (width - 400, height - 400)
# Rendering Weapons
    SCREEN.blit(bow_img, (bow_pos))
    ...
    sword_y = sword_img.get_height()
    spear_x = spear_img.get_width()
    spear_y = spear_img.get_height()
    spell_x = spell_img.get_width()
...
def play():
    ...
    background1 = pygame.image.load("images/backgroundtest.png")
    background1 = pygame.transform.scale(background1, (width, height))
    background2 = pygame.image.load("images/backgroundtest2.png")
    ...
    while True:
        ...
        if pygame.mouse.get_pressed()[0]:
            for enemy in enemies:
                if enemy.health > 0 and player_pos.distance_to(enemy.pos) <= player_range:
                    if current_time - last_atk_time >= atk_cooldown / player_atk_speed:
        ...
        # elif level >= 2:
        #     SCREEN.blit(background2, (width,height))
...
def main_menu():
    global menu_music_tracks, music_volume
    current_menu_track = random.choice(menu_music_tracks)
    print(current_menu_track)
    try:
